...
def copy_asset(src, dest):
    ...
    shutil.copy2(src, dest); print("copy", src, "->", dest)

def main():
    if not MANIFEST.exists():
        print("ERROR: publish.yaml no trobat", file=sys.stderr); sys.exit(1)
    ...
    for it in items:
        ...
        for dl in it.get("downloads",[]) or []:
            ...
            dl_links.append(("assets/"+str(rel)).replace("\","/"))

        sections[it.get("section","Altres")].append({
            "title": title, "page": str(pathlib.Path(out_rel)).replace("\","/"),
            "downloads": dl_links
        })
